...
install:
	@docker run --rm \
		...
		grandchef/composer:2.8.1 install --ignore-platform-reqs --no-scripts --no-interaction
...
update:
	@docker run --rm \
		...
		-v /etc/passwd:/etc/passwd:ro \
		-v /etc/group:/etc/group:ro \
		grandchef/composer:2.8.1 update --no-scripts --no-interaction
...
autoload:
	@docker run --rm \
		...
		-v /etc/passwd:/etc/passwd:ro \
		-v /etc/group:/etc/group:ro \
		grandchef/composer:2.8.1 dump-autoload --no-scripts --no-interaction
...
cmd:
	@docker run --rm -it \
		...
		-v /etc/group:/etc/group:ro \
		-w /app \
		grandchef/php:8.1.0-fpm-dev /bin/bash
